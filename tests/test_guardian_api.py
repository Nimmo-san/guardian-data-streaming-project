...
class TestGuardianAPI:
    ...
    def test_fetch_guardian_articles_invalid_api_key(self, mock_requests_get):
        ...
        mock_requests_get.return_value.json.return_value = {"response": {"results": []}}
        fetch_guardian_articles(guardian_api_key=invalid_api_key, search_term="machine learning", date_from="2025-01-01", logger=mock_logger)

        mock_logger.error.assert_called_with(f"Received response - status code: {403}")
